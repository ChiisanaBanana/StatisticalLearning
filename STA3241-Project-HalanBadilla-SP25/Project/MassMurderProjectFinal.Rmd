...
```{r}
# This will be hot encoded for the other models
...
```{r}
# Holdout Test Data
# Need two as one is factored and the other is hot encoded. They use the same predictors
...
```{r}
lmModel <- lm(DTIRatioLog ~ .-incident_id, data = trainData)
...
```{r}
# Logistic Regression (Elastic Net, Ridge + Lasso)
...
elasticMetricsF <- postResample(elasticPredF, testDataFactored$DTIRatio)
```
...
```{r}
# XGBoost
xbgModelE <- train(DTIRatioLog ~ ., data = trainDataEncoded, method = "xgbTree", trControl = train_control, tuneLength = 10)
xgbModelE <- xbgModelE # NO WAY am I rerunning this. With the training it took roughly 45 minutes. I do not have the time for that so I am just copying the results to this instead of rerunning it.
print(xgbModelE)
```
...
-RSquared: 79% of the variance was explained by the model.
...
-RSquared: Only ~35% of the variance is explained. Much worse than hot-encoded.
-MAE: Higher MAE is worse than hot-encoded. In general the predictions are even less accurate.

RF Factorized:
-RMSE: Quite low, lower than ENR H-E, quite strong predictions. Fits the data rather well.
-RSquared: 88% of the variance was explained, better than any other so far.
-MAE: Very low MAE shows that it makes a lot less errors on average when it predicts.

XGBoost Hot-Encoded:
-RMSE: The lowest RMSE. The most accurate out of the bunch.
-RSquared: Very high, captures ~94% of the variance.
-MAE: EXTREMELY low MAE. The closest to the actual values with VERY little error.

XGBoost is by far the best model here, this would be a good model to use when predicting for the fatality rate (Death to Injury+Death Ratio). Of course, in a real world setting and if you need very fast computations, RF model would be the way to go. RF predicted very well with only a fraction of the time needed whereas XGBoost gave much better results but took about 6-7 times as long to compute. Of course, this would depend on your processing speed but even with my setup, (6 Core 4.2GHz CPU and with 32GB DDR4 1577 RAM) it took roughly 45 minutes to run a XGBoost with a tuneLength of 10 and train CV cycle of 5. It needed to run 50 different simulations taking around ~54 seconds per iteration. This is on the faster end and realistically may not be viable if this model is to be applied to a situation in which speed is a priority or the computational resources are not sufficient.

In conclusion:
Elastic Net Regression (With Hot-Encoded Variables): Is the quickest but drops the most in accuracy.
RF: The model to choose when you need quicker but accurate results.
XGBoost: The best model but takes much longer.

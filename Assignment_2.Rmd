...
![My work](C:/Users/halan/Desktop/FPU/Spring2025/Statistical Learning/Problem1.JPG)
...
```{r}
auto <- read.csv("C:/Users/halan/Desktop/FPU/Spring2025/Statistical Learning/Auto.csv")
...
```{r}
rockData <- read.table("C:/Users/halan/Desktop/FPU/Spring2025/Statistical Learning/rock_aquifer.csv")
rockData
```
...
vif
```
3.16 is generally a pretty good amount.
...
```{r}
regSumLog <- summary(regFullLog)
regSumLog$cp
regSumLog$bic
regSumLog$adjr2
```
```{r}
which.min(regSumLog$cp)
which.min(regSumLog$bic)
which.max(regSumLog$adjr2)
```

```{r}
permeabilityMod <- lm(`Log Permeability` ~ Porosity + `Insoluble Residue` + `Grain Length of a-axis` + `SD of a-axis length` + `Grain Length of b-axis` + `SD of b-axis length` + Calcite + Dolomite, rockData)
summary(permeabilityMod)
```
I just chose 8 since it was right between the two. That said, 6 could be argued to be better as it showed all having some sort of significance (though sample was a categorical var but it could still mean something) yet with 8 it doesn't show "SD of a-axis length" too have any meaningful significance. Given this was the middle of the 3 tests I still ended up going with this one and I find it to be the most balanced.

```{r}
1/(1-0.6415)
```
Decent VIF, better than the previous one (less multi-coll) but not as good as what is in the solutions. I think it's a drawback I'd take though.

...
```{r}
varI <- incidents %>%
  select(c(longitude, latitude, firstcod, situation_type, location, num_victims_killed, num_victims_injured)) %>%
...
ggplot(stateIncidents, aes(x = reorder(state, +n), y = n, fill = fillColor)) + 
  ...
  scale_fill_identity() +
  scale_fill_manual(values = c("0-19" = "#0a6f97", "20-39" = "#5c0a97", "40+" = "#970a0c")) +
...
```{r}
ggplot(deadlynessPerState, aes(x = reorder(state, +totalDeaths), y = totalDeaths)) +
  geom_bar(stat = 'identity', width = 0.8) +
  scale_fill_manual(values = c("0-99" = "#0a6f97", "100-199" = "#5c0a97", "200+" = "#970a0c")) +
  theme(axis.text.y = element_text(size = 7, hjust = 1)) +
  labs(title = "Total Victim Deaths Per State", x = "States", y = "Amount of Deaths") +
  coord_flip()

ggplot(deadlynessPerState, aes(x = reorder(state, +totalInjured), y = totalInjured)) +
  geom_bar(stat = 'identity', width = 0.8) +
  scale_fill_manual(values = c("0-99" = "#0a6f97", "100-199" = "#5c0a97", "200+" = "#970a0c")) +
  theme(axis.text.y = element_text(size = 7, hjust = 1)) +
